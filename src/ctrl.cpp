...
void initeoshlle3f_(char* filename, double* B, double* volex0, double* delta0,
                    double* aaa, double* bbb) {
  eos = new EoS3f(filename, *B, *volex0, *delta0, *aaa, *bbb);
}

void initeoshlle1f_(char* filename) { eos = new EoS1f(filename); }

void inittrcoeff_(double* etaS, double* zetaS) {
  trcoeff = new TransportCoeff(*etaS, *zetaS, eos);
}

void eoshlle_(double* e, double* nb, double* nq, double* ns, double* T,
              double* mub, double* muq, double* mus, double* p) {
  eos->eos(*e, *nb, *nq, *ns, *T, *mub, *muq, *mus, *p);
}

void eosrangeshlle_(double* emax, double* e0, double* nmax, double* n0, int* ne,
                    int* nn) {
  eos->eosranges(*emax, *e0, *nmax, *n0, *ne, *nn);
}

void initfluidhlle_(int* nx, int* ny, int* nz, double* minx, double* maxx,
                    double* miny, double* maxy, double* minz, double* maxz) {
  f = new Fluid(eos, trcoeff, *nx, *ny, *nz, *minx, *maxx, *miny, *maxy, *minz,
                *maxz);
}
...
void icgethlle3f_(double* x, double* y, double* eta, double* e, double* nB,
...
void icgethlle_(double* x, double* y, double* eta, double* e, double* nB,
                double* nQ, double* nS, double* vx, double* vy, double* vz) {
  double nu, nd, ns;
  ic_kw->getICs(*x, *y, *eta, *e, *vx, *vy, *vz, nu, nd, ns);
  *nB = *nQ = *nS = 0.;
...
void icsethlle_(int* ix, int* iy, int* iz, double* tau0, double* e, double* nb,
                double* nq, double* ns, double* vx, double* vy, double* vz) {
  Cell* c = f->getCell(*ix - 1, *iy - 1, *iz - 1);
  if ((*vx) * (*vx) + (*vy) * (*vy) + (*vz) * (*vz) > 1.) {
    //	cerr << "setIC : " << ix << "  " << iy << "  " << iz << "  e = " << e <<
    //"  v^2 = " << (*vx)*(*vx)+(*vy)*(*vy)+vz*vz << endl ;
    double factor = sqrt((*vx) * (*vx) + (*vy) * (*vy) + (*vz) * (*vz));
    (*vx) = (*vx) * 0.99 / factor;
    (*vy) = (*vy) * 0.99 / factor;
    (*vz) = (*vz) * 0.99 / factor;
  }
  if (isinf(*e) || isnan(*e) || isinf(*nb) || isnan(*nb) || isinf(*nq) ||
      isnan(*nq) || isinf(*ns) || isnan(*ns) || isinf(*vx) || isnan(*vx) ||
      isinf(*vy) || isnan(*vy) || isinf(*vz) || isnan(*vz)) {
    cout << "fluid: bad init," << setw(14) << "e" << setw(14) << "nb"
         << setw(14) << "nq" << setw(14) << "ns" << setw(14) << "vx" << setw(14)
         << "vy" << setw(14) << "vz" << endl;
    cout << "----------------" << setw(14) << e << setw(14) << nb << setw(14)
         << nq << setw(14) << ns << setw(14) << vx << setw(14) << vy << setw(14)
         << vz << endl;
...
void dtauhlle_(double* dtau) { h->setDtau(*dtau); }
...
int getmaxstephlle_(void) { return maxstep; }
...
void getvalueshlle_(int* ix, int* iy, int* iz, double* e, double* p, double* nb,
                    double* nq, double* ns, double* vx, double* vy, double* vz,
...
void getvflaghlle_(int* ix, int* iy, int* iz, double* viscCorrCutFlag) {
  *viscCorrCutFlag =
      f->getCell(*ix - 1, *iy - 1, *iz - 1)->getViscCorrCutFlag();
}
...
double getxhlle_(int* ix) { return f->getX(*ix - 1); }

double getyhlle_(int* iy) { return f->getY(*iy - 1); }

double getzhlle_(int* iz) { return f->getZ(*iz - 1); }
...
void destroyeoshlle_(void) { delete eos; }
  ...
  for (int ix = 0; ix < f->getNX(); ix++)
    for (int iy = 0; iy < f->getNY(); iy++)
      for (int iz = 0; iz < f->getNZ(); iz++) {
        f->getCell(ix, iy, iz)
            ->getPrimVar(eos, tau, e, p, nb, nq, ns, _vx, _vy, _vz);
