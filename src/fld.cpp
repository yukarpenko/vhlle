...
void Fluid::outputGnuplot(double time) {
 ...
 for (int ix = 0; ix < nx; ix++) {
  ...
  foutx << setw(14) << time << setw(14) << x << setw(14) << vx << setw(14) << vy
 ...
 for (int iy = 0; iy < ny; iy++) {
  ...
  c->getPrimVar(eos, e, p, nb, nq, ns, vx, vy, vz);
  eos->eos(e, nb, nq, ns, t, mub, muq, mus, p);
  fouty << setw(14) << time << setw(14) << y << setw(14) << vy << setw(14) << vx
        << setw(14) << e << setw(14) << nb << setw(14) << t << setw(14) << mub;
 ...
 for (int iz = 0; iz < nz; iz++) {
  ...
  getCell(nx / 2, ny / 2, iz)->getPrimVar(eos, e, p, nb, nq, ns, vx, vy, vz);
  eos->eos(e, nb, nq, ns, t, mub, muq, mus, p);
  foutz << setw(14) << time << setw(14) << z << setw(14) << vz << setw(14) << vx
        << setw(14) << e << setw(14) << nb << setw(14) << t << setw(14) << mub;
 ...
 for (int ix = 2; ix < nx - 2; ix++)
  for (int iy = 2; iy < ny - 2; iy++)
   for (int iz = 2; iz < nz - 2; iz++) {
    ...
    if (trcoeff->isViscous()) Efull += c->getpi(0, 0);
    if (e > 1e-10) {
     nCoreCells++;
    ...
    vxvy_den += e;
    txxyy_num +=
        (e + p) / (1. - vx * vx - vy * vy - vz * vz) * (vx * vx - vy * vy);
    txxyy_den +=
        (e + p) / (1. - vx * vx - vy * vy - vz * vz) * (vx * vx + vy * vy) +
        2. * p;
    pi0x_num += e / (1. - vx * vx - vy * vy - vz * vz) * fabs(c->getpi(0, 1));
...
#endif
     double dEsurfVisc = 0.;
     for (int i = 0; i < 4; i++) dEsurfVisc += piC[index44(0, i)] * dsigma[i];
  ...
  for (int iy = 2; iy < ny - 2; iy++)
   for (int iz = 2; iz < nz - 2; iz++) {
    ...
    if (isCorona && !isTail) {
     nelements++;
     ffreeze.precision(15);
     ffreeze << setw(24) << time << setw(24) << getX(ix) + 0.5 * dx << setw(24)
