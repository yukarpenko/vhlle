...
statustab = new int [ne*nnb*nnq] ;
 ...
 if(fabs((e0*exp(logemin)-emin)/emin)>1e-5 || fabs((e0*exp(logemax)-emax)/emax)>1e-5 || fabs(n0*exp(lognmax)-nbmax)>1e-4)
   { cout << "EoSHadron: wrong eps or nb range: "<<setw(14)<<emin<<setw(14)<<emax<<setw(14)<<nbmin<<setw(14)<<nbmax<<endl ; exit(1) ; }
		...
		double &T, double &mub, double &muq, double &mus, double &p)
{
 ...
 if(statustab[index3(ie,inb,inq)]==1) { T = mub = muq = mus = p = 0. ; return ; }
