...
{
	double val ;
	if(e<0.1270769021427449) val = 12.2304*pow(e,1.16849) ;
	else if(e<0.4467079524674040) val = 11.9279*pow(e, 1.15635) ;
	else if(e<1.9402832534193788) val = 0.0580578 + 11.833*pow(e, 1.16187) ;
	else if(e<3.7292474570977285) val = 18.202*e - 62.021814 - 4.85479*exp(-2.72407e-11*pow(e,4.54886))
	     + 65.1272*pow(e,-0.128012)*exp(-0.00369624*pow(e,1.18735)) - 4.75253*pow(e,-1.18423) ;
	else val = 18.202*e - 63.0218 - 4.85479*exp(-2.72407e-11*pow(e,4.54886))
...
void loadSongIC(char* filename, double factor)
{
 ...
 for(int iy=0; iy<NY; iy++){
	...
	if(glauberVariable==1) e[ix][iy] = s95p_e(factor*e[ix][iy]) ;
	else e[ix][iy] = factor*e[ix][iy] ;
 }
