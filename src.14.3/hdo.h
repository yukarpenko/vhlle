...
private :
	...
	double tau_z ; // effective value of the proper time used in 1/tau factors in the fluxes. Used to increase the accuracy 
public :
  ...
  // ideal source terms, output : S[7],  input: the rest of the parameters
	void source(double tau, double x, double y, double z, double e, double p, double nb, double nq, double ns,
  ...
  void setNSvalues() ;
  // advances numerical solution for shear/bulk in a whole grid over one timestep
	void ISformal() ;
	void setQfull() ;
  // advances numerical solution for Q (including ideal and viscous fluxes and source terms) over one timestep
