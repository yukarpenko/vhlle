...
void EoS3f::getun(double n, int &ixn, double &un){
        ...
        k1 = 0 ;
        k2 = nn-1 ;
        for(;k2-k1>1;){k=(k1+k2)/2 ; if(n<ngrid[k])k2=k ; else k1=k ;}
        ixn=k1;
		...
		double &_T, double &_mub, double &_muq, double &_mus, double &_p)
{
	...
	for(int jns=0; jns<2; jns++){
		_p += we[je]*wnb[jnb]*wnq[jnq]*wns[jns]*pre[index(ixe+je,ixnb+jnb,ixnq+jnq,ixns+jns)] ;
		_T += we[je]*wnb[jnb]*wnq[jnq]*wns[jns]*T[index(ixe+je,ixnb+jnb,ixnq+jnq,ixns+jns)] ;
		_mub += we[je]*wnb[jnb]*wnq[jnq]*wns[jns]*mub[index(ixe+je,ixnb+jnb,ixnq+jnq,ixns+jns)] ;
		_muq += we[je]*wnb[jnb]*wnq[jnq]*wns[jns]*muq[index(ixe+je,ixnb+jnb,ixnq+jnq,ixns+jns)] ;
		_mus += we[je]*wnb[jnb]*wnq[jnq]*wns[jns]*mus[index(ixe+je,ixnb+jnb,ixnq+jnq,ixns+jns)] ;
	}
	if(_p<0.) _p = 0. ;
