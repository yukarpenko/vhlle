...
using namespace std ;
 ...
 double ***e, ***nb, ***nq, ***vx, ***vy, ***vz, xmin, xmax, ymin, ymax, zmin, zmax ;
...
void loadIC(char* filename)
{
 ...
 for(int iz=0; iz<NZ; iz++){
	fin >> x[ix] >> y[iy] >> z[iz] >> e[ix][iy][iz] >> nb[ix][iy][iz] >> nq[ix][iy][iz] >> vx[ix][iy][iz] >> vy[ix][iy][iz] >> vz[ix][iy][iz] ;
 }
 xmin = x[0]; xmax = x[NX-1] ;
 ymin = y[0]; ymax = y[NY-1] ;
 zmin = z[0]; zmax = z[NZ-1] ;
 cout<<"ICUrQMD: table read, limits are "<<xmin<<"  "<<xmax<<"  "<<ymin<<"  "<<ymax<<"  "<<zmin<<"  "<<zmax<<endl ;
 delete [] x ;
 delete [] y ;
 delete [] z ;
}

void getIC(double x, double y, double eta, double &eout, double &nbout, double &nqout, double &vxout, double &vyout, double &vzout)
{
 ...
 if(ix<0 || ix>NX-2 || iy<0 || iy>NY-2 || iz<0 || iz>NZ-2)
   { eout = vxout = vyout = vzout = 0.0 ; return ; }
