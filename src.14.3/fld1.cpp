...
//----- Cornelius stuff
  ...
  double eCube[2][2][2][2], pCube[2][2][2][2], nbCube[2][2][2][2] ;
  ...
  for(int isegm=0; isegm<Nsegm; isegm++){
    ...
    double vxC=0., vyC=0., vzC=0., TC=0., mubC=0., muqC=0., musC=0., piC[10], PiC=0., nbC=0. ; // values at the centre, to be interpolated
    double eC=0., pC=0. ;
    ...
    for(int jz=0; jz<2; jz++){
     ...
     musC +=musCube[jt][jx][jy][jz]*wCenT[jt]*wCenX[jx]*wCenY[jy]*wCenZ[jz] ;
      ...
      nbC += nbCube[jt][jx][jy][jz]*wCenT[jt]*wCenX[jx]*wCenY[jy]*wCenZ[jz] ;
    }
    //double Teos ;
