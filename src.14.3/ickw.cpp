...
void IC_KW::getICs(double x, double y, double z, double &_e, double &_vx, double &_vy, double &_vz, 
...
//	 cout << "di " << di << " dj " << dj << " dk" << dk << " e " << vz[ix+di][iy+dj][iz+dk] << endl ;

//	cout << " e = "	<< e[ix][iy][iz] << endl ;
	_e = 0. ; _vx = 0. ; _vy = 0. ; _vz = 0., _nu = 0. ; _nd = 0. ; _ns = 0. ;
...
for(int iz=0; iz<f->getNZ(); iz++){
  ...
  getICs(x,y,eta,e,vx,vy,vz,nu,nd,ns) ;
	if(e<0.) { e = nu = nd = ns = vx = vy = vz = 0. ; }
  if(vx*vx+vy*vy+vz*vz>1.){
//	cout << "setIC : " << ix << "  " << iy << "  " << iz << "  e = " << e << "  v^2 = " << vx*vx+vy*vy+vz*vz << endl ;
...
/*    double  ee, pp, nbx, nqx, nsx, vxx, vyx, vzx ;
    ...
    cout << "+++++++++++++++ e_in  = "<< e << endl;
    cout << "+++++++++++++++ e_out = "<< ee << endl;
    cout <<ix<<" "<<iy<<" "<<iz<<" "<<e<<" "<<nb<<" "<<nq<<" "<<ns<<" "<<vx<<" "<<vy<<" "<<vz<<endl;
    cout <<ee<<" "<<pp<<" "<<nbx<<" "<<nqx<<" "<<nsx<<" "<<vxx<<" "<<vyx<<" "<<vzx<<endl;
    exit(1);
    }
*/
  }
